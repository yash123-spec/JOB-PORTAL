...
const deleteJobByAdmin = asyncHandler(async (req, res) => {
  ...
  // public_id = everything after "/upload/" minus the version segment; raw resources keep the extension.
  const publicIds = applications
    .filter(app => app.resumeUrl)
    .map(app => {
      const afterUpload = app.resumeUrl.split('/upload/')[1];
      return afterUpload ? afterUpload.replace(/^v\d+\//, '') : null;
    })
    .filter(Boolean);
  ...
  if (publicIds.length > 0) {
    try {
      ...
      console.warn(`Failed to batch-delete resumes from Cloudinary: ${error.message}`);
    }
  }
