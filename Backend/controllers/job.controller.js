...
const getAllJobs = asyncHandler(async (req, res) => {
  ...
  // ─── Step 3: Get total count on SAME filtered dataset ────────────
  // Run pipeline up to this point (before skip/limit) and count
  const countPipeline = [...pipeline, { $count: "total" }];
  const countResult = await Job.aggregate(countPipeline);
  const totalJobs = countResult[0]?.total || 0;

  // ─── Step 4: Add sort + pagination at the END ────────────────────
  pipeline.push(
    { $sort: { createdAt: -1 } },
