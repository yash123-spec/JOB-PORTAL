...
const MessagesIcon = () => {
    ...
    // Load once on mount / login. We deliberately DON'T depend on the route:
    // the socket "message:new" listener below keeps the unread count live, so
    // re-fetching on every page navigation was redundant network traffic.
    useEffect(() => {
                            ...
                            conversations.map((conv) => {
                                ...
                                return (
                                    ...
                                    >
                                        {/* Avatar */}
                                        {other?.profilePic ? (
                                            <img src={optimizedImage(other.profilePic, { width: 80 })} alt={other.fullname} loading="lazy" className="w-10 h-10 rounded-full object-cover flex-shrink-0" />
                                        ) : (
